...
void FileList::emplaceBack(const char *name, const char *date) {
...
void FileList::addFile(const char *name, const char *date) {
    int back = 0;
    int front = 0;
    if (!files_cnt) {
        emplaceBack(name, date);
        return;
    }
...
const char *FileList::findByName(const char *name) const {
    int back = 0;
    int front = 0;
    if (!files_cnt) {
        return nullptr;
